...
// this is the main method of recursion for each node.
public class ProxyMethodInvocation implements MethodInvocation{
    private List<MethodInterceptor> interceptors;
    private int index;
    private Object originObj;
    private Method originMethod; // the method at the moment(now)
    private Object[] args;
    ...
    public Object proceed() throws Throwable {
        if (index >= interceptors.size()) { // if index exceed the size of interceptors, it will run the original method for going next element.
            return executeOriginalMethod();
        }
        MethodInterceptor interceptor = interceptors.get(index++);
        return interceptor.invoke(this); // the Invocation of current interceptor
    }
...
/**
 *    explanation of AOP (aspect oriented programming)
 *    [afterItc, beforeItc]
 *
 *    afterItc.invoke(mi)
